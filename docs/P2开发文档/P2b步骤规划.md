...
**来源**：id Software（DOOM, 1993）、任天堂 EPD（塞尔达传说：荒野之息, 2017）。

**核心主张**：游戏逻辑必须能在不启动引擎、不挂载 UI、不依赖渲染的条件下独立运行和测试。表现层只读取逻辑层的状态然后画在屏幕上。

**本项目落地**：

- GraphEngine（`@my-project/graph-engine`）是框架无关的纯函数包——不依赖 Pinia / Vue / Cytoscape。119 个测试在 vitest 里直接跑，不启动浏览器。
...
**来源**：Vlambeer（《Nuclear Throne》《Luftrausers》）。

**核心主张**：日常 UI / 交互 / 视觉参数的迭代循环（改值 → 验证 → 确认）必须在 30 秒内完成，否则心流被打断，迭代效率骤降。这并不意味着完整测试套件要在 30 秒内跑完——它只要求"亲眼看效果"这个循环足够短。
...
**为什么不是"浪费时间"**：如果没想清楚"用户点 Orbit 按钮后光标应该变成什么样"就写代码，就会反复改——每次 HMR 虽然不到 1 秒，但改二十次是二十次心流打断。架构先在脑子里搭好，代码写一次就到位，这才是真正的快速迭代。

**本项目落地**：Phase 2b 的 Step 1（交互模式架构设计与反馈规范）即在写代码前完成 Cognition 和 Arrangement 两种模式下全部操作的 UI 交互流程定义，包含：模式切换方式、子工具选择、用户每步操作顺序、前端即时反馈、确认/取消机制、完成状态、错误状态。

### 4. 每个子系统有唯一的质量负责人

**来源**：Monolith Soft（《异度神剑》系列）、Supergiant Games（《哈迪斯》系列）。

**核心主张**：每个子系统必须有**一个人**对其质量负全责。这个人是决策者也是执行者，设计意图不需要经过文档、会议、工单来流转——写代码的人就是定设计的人。好处是全栈理解力不被组织结构割裂。

**本项目落地**：单人项目天然满足——全部子系统（引擎类型系统、执行器、编排层、前端 Store、渲染层、UI 组件）由同一人端到端负责。上限是项目规模不能超过"一个人能同时理解全部子系统"的认知边界——当前约 7,600 行有效代码，在这个边界内。

### 5. 部分验证先行——在投入全量实现前用最小成本检验设计假设
