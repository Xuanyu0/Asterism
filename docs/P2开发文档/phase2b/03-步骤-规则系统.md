...
**目标**：安装 Lucide 图标库并用 SVG 图标替换文本符号。

**改动范围**：

| 文件 | 改动 |
|------|------|
| `frontend/package.json` | 新增 `lucide-vue-next` 依赖 |
| `frontend/src/components/graph/OperationToolbar.vue` | 替换 `standingButtons[].icon` 为 Lucide 组件引用 |
