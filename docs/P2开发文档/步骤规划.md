...
**跨图补齐**：`graph_registry.ts` 就绪后，回填 `execute.ts` 中的跨图守卫——`executeAddEdge` 的端点穿透、`executeDeleteNode` 的级联删除、`executeUpdateNode` 的 label 穿透。届时 `sourceGraphId !== graph.id` 的条件分支改为通过 registry 查找源图并执行同步。
...
| 5.3 | `collision.ts` | 三个原语。**`constrainPosition`**：单点单帧碰撞约束，拖拽实时调用——desired 无重叠则原样返回，有重叠则沿接触法向推至表面 + ε，O(n) 每帧。**`computePath`**：两点间无碰撞路径规划——直线可行直接返 `[to]`；被挡则尝试切向绕行（障碍圆左右两侧）；绕不过则返回 `unreachable`。**本模块不修改其他节点位置**——路径规划是只读几何搜索，不推动障碍物。**`untangleCluster`**：批量解绕——迭代逐对检测并沿法向推开至收敛。虚节点固定 r₀ 全部原语中特殊处理 | ~110 行 |
...
| # | 任务 | 说明 |
|---|------|------|
| 7.1 | `move.ts` | 批量移动编排。对每个节点调 `computePath(from, to)` 生成路径点序列，组装为多帧 `move_node` 操作序列。若任一节点路径不可达则整批搁置 |
| 7.2 | `adjust.ts` | `computeAdjustedDistance` / `computeAdjustedAngle` — 纯三角函数，给定两点 + 参数，返回新坐标。计算后**不**单独调碰撞——由调用方在路径规划中统一约束 |
| 7.3 | `path.ts` | `computePathLayout`——指定轴方向 + 间距 → 逐节点计算位置。每步调 `constrainPosition` 保证产出无重叠。碰撞是计算的**输入条件**，不是后处理 |
