...
| 9 | 测试覆盖 | 🟡 中 ⏳ | ~800 行 | 19 个测试文件。spec 边界场景已标注，待实现 |
| 10 | 公开 API 收口 | 🟢 低 ⏳ | ~20 行 | index.ts 已有初步导出结构，待按消费者分类标注 |
| 11 | **前端适配** | 🟡 中 ⏳ | ~120 行改动 | graph_store 退化 + undo/redo + 操作日志接入 |
...
| # | 任务 | 应覆盖的边界场景 |
|---|------|---------|
| 9.1 | `core/validate.test.ts` | 11 种 Operation 的校验路径：合法 / ID 重复 / 不存在 / 超限 |
| 9.2 | `core/execute.test.ts` | 11 种 Operation 的执行路径：add_node / add_edge / delete_node（含级联删除引用节点）/ delete_edge / update_node（含 label 穿透）/ update_edge / move_node / collapse_dependency / expand_dependency / add_graph（registerGraph 调用）/ delete_graph（unregisterGraph 调用） |
| 9.3 | `core/reversal.test.ts` | 11 种 Operation 的逆操作构造 + 逆操作执行后状态与操作前一致 |
| 9.4 | `core/replay.test.ts` | 给定基线和操作序列，回放到指定步数后状态正确 |
| 9.5 | `core/sync.test.ts` | `syncReferenceNodeDegree`：源节点 degree 变更 → 同图引用节点跟随；跨图引用跳过（由镜像负责） |
| 9.6 | `infrastructure/graph_registry.test.ts` | Map CRUD、多图并存、覆盖注册、注销不存在 ID |
| 9.7 | `infrastructure/search.test.ts` | 匹配、跨图搜索、空结果、graphId 过滤、graphPath 回溯 |
| 9.8 | `infrastructure/collision.test.ts` | 无碰撞、有碰撞、推开后验证（已完成：18 测试） |
| 9.9 | `infrastructure/placement.test.ts` | positionOnCircle / snapOrbit / distributeOnTiers（约束 A/B/C）/ distributeOnLine / scatterInCircle（均匀分布）/ computeTierSpacing 六函数 |
| 9.10 | `core/checkers/rule_checkers.test.ts` | 6 项原子规则 + 引用节点/虚节点新增规则 |
| 9.11 | `compose/pipeline.test.ts` | applyBatch 事务语义（全通过 / 部分失败 / dryRun / stopOnFirst）+ add_graph / delete_graph 经 registry 执行 |
| 9.12 | `compose/arrangement/move.test.ts` | 单节点移动：无碰撞通过、碰撞返回 error、空节点列表 |
| 9.13 | `compose/arrangement/orbit.test.ts` | 初始吸附（保留角度）+ 碰撞检测 + 无边 error + tier 覆盖校验 |
| 9.14 | `compose/arrangement/path.test.ts` | 直线排列 + 碰撞检测触发 + 无边 error |
| 9.15 | `compose/arrangement/adjust.test.ts` | Adjust Distance（连续）/ Adjust Orbit（离散吸附 + 碰撞） |
| 9.16 | `compose/cognitive/deconstruct.test.ts` | 正常解构（含邻居）+ 无邻居节点 + 虚节点拒绝 + 抽象节点拒绝 + 非 knowledge 拒绝 |
| 9.17 | `compose/cognitive/induce.test.ts` | 2 节点归纳 + 含启发节点参与 + 沟通节点拒绝 + 重边冲突 + 内部边下沉 + 多外部邻居 + 沟通节点位置正确 |
| 9.18 | `compose/cognitive/internalize.test.ts` | 原子节点 + 抽象节点（含子图 DFS 清理）+ 纯引用节点（全部自动删除）+ 混合（知识+引用）+ scatterInCircle 位置递增 |
| 9.19 | `compose/cognitive/diverge.test.ts` | Case A（含单 reference 合法）+ Case A 含两 reference 拒绝 + Case B（启发创建）+ Case B 镜像 + scatterInCircle 位置 |
...
| # | 任务 | 说明 |
|---|------|------|
| 10.1 | 按 6 类重新组织 `engine/src/index.ts` 导出结构 | 当前分类与 spec 不一致，改为 apply / applyBatch / replay / reversal / compose / infrastructure |
| 10.2 | 为每个导出函数标注消费者 | `/** 消费者：graph_store */` 或 `/** 消费者：operation_controller */` |
| 10.3 | 确认不导出列表 | 验证 execute / validate / sync / checkers / collision / placement / geometry 均不出现在 index.ts |
