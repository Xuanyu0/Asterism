...
| 12 | **操作日志 + 回溯 UI** | 🟡 中 | — | 待定。undo/redo 需独立 spec，由用户编写后再执行 |
...
### Step 12：操作日志 + 回溯 UI（待 spec 定稿后执行）

> **状态**：⏸️ 暂缓。undo/redo 涉及操作树多分支选择、cursor 遍历语义、UI 交互细节，需用户编写 spec 后再执行。
>
> **前置依赖**：Step 11 全部完成（前端已切到引擎 `apply()` 路径）。

| # | 任务 | 说明 |
|---|------|------|
| 12.1 | `graph_store.ts` 接入操作日志（仅 undo） | `undoStack` 升级为 `OperationLog`（`{ entries, cursor }`）。每次 `apply()` 前调 `createReversal` 捕获逆操作，执行后追加 `OperationLogEntry`。实现 `undo()` 方法：沿 `parentIndex` 链回溯——每步执行当前 entry 的 `reversalOperations`，cursor 跳到 `parentIndex`。**redo 延后**：操作树是树结构，cursor 处可能有 ≥2 个子节点（分支），redo 需分支选择 UI，不在本次范围。`replayGraph` 接入历史回顾流程同步延后 |
| 12.2 | 左下角回溯按钮（仅 ←） | 见 `04-UI与笔记库.md`。← 调 `undo()`。cursor 到达根节点（`parentIndex === -1`）时按钮变灰。按钮无效时不响应点击。**→ 按钮延后**：与 12.1 的 redo 延后原因相同——多分支选择 UI 待设计 |
