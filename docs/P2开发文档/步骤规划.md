...
| 7.5 | **认知操作 Spec** | 🟢 低 | ~80 行文档 | 为 Step 8 的 4 个认知操作编写精确规格说明书（输入/前置/后置/原子操作序列/事务语义）。确保代码开工前所有行为已穷举，不留"边写边决策"的敞口 |
| 8 | 认知操作层 | 🔴 高 | ~310 行新增 | induce 的跨图事务一致性、internalize 递归子图处理、diverge 用 scatterInCircle 自动完成镜像节点位置。依赖 Step 7 的布局编排和 Step 7.5 的 Spec |
...
### Step 7.5：认知操作 Spec 编写（docs/specs/）
...
1. 认知操作序列的合法性判定粒度是什么——整个认知操作合法/不合法，还是序列中每个原子操作各自判？跨多图时 `applyBatch` 的 scope 如何定义？
    * 答案：总体上，逐个检查每个操作，但只要有一个操作非法，那么整批操作就不合法，也就是“一票否决”。
2. internalize 递归子图中的沟通节点（指向父图节点）在搬运后应删除还是保留？
    * 答案：根据常识层没有边，且沟通节点的存在依靠抽象节点于其他节点的连接，所以沟通节点在一块内化后到常识层中一并删除
3. deconstruct 的原有边应保留在父图还是下沉到子图？
    * 答案：不仅保留在父图中，也下沉复制到子图
4. induce 被选节点与未选节点之间的外部边，每条外部边一个沟通节点，还是多个未选节点可共享一个沟通节点？
    * 答案：如果原图中，未选节点被多个被选节点共用，那么多个知识节点共享同一个沟通节点。
5. diverge 引擎函数是否假设搜索已完成——接收已确定的节点 ID 和图 ID，不负责交互式搜索？
    * 答案：diverge不负责交互式操作，只接受确定的节点 ID 和图 ID，搜索逻辑交给前端完成。
6. deconstruct 空子图是否自动创建沟通节点？
    * 答案：是。
7. commonLayer 的 parentGraphId 如何设置？
    * 答案：就是主图谱。
8. internalize 后，子图内的普通边怎么办？
    * 答案：子图中的节点位置不变，普通边全删。
9. induce 外部边投影时，重边冲突怎么办？
    * 答案：暂时禁止这样的操作结果的发生。
10. deconstruct 下沉复制边时，子图里复制的是哪些边？
    * 答案：复制抽象节点领居的边，且领居作为子图的沟通节点。
11. diverge 生成启发节点时是否允许重复？
    * 答案：允许。
12. diverge 的镜像操作是否属于同一个事务？
    * 答案：是，采用“一票否决”。
13. diverge 的有向虚边方向如何定义？
    * 答案：用户在前端先确认source，再确认target。
14. 认知操作是否全部需要 undo snapshot？
    * 答案：支持，且作为OperationLog中一个“批量操作”的对象。

**设计完整度评估**：

| 操作 | 完整度 | 已决策 | 残留 |
|------|:------:|--------|------|
| `deconstruct.ts` | **92%** | A2 边继承规则（保留父图 + 下沉复制）、空子图自动创建沟通节点、复制邻居边且邻居作为沟通节点 | Q15：复制边的方向/类型是否保留原样？ |
| `induce.ts` | **78%** | 外部边共享沟通节点、禁止重边冲突、一票否决 | Q16：内部边跟随下沉还是留在父图？Q17：抽象节点在父图中是否直接参与边结构？Q18：被选节点移入子图后旧位置留空？ |
| `internalize.ts` | **88%** | 沟通节点删除、普通边全删、节点位置不变 | — |
| `diverge.ts` | **93%** | 不负责搜索、允许重复启发节点、镜像操作同一事务一票否决、有向虚边方向由前端定、scatterInCircle 自动放位 | — |
| **整体** | **88%** | — | 3 个残留问题（Q15–Q18），均为实现细节级 |
