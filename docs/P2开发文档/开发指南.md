...
* 模型：Git 追加模型
  ...
  * cursor 移动定位当前状态
  * 旧分支通过 reflog 保留引用
* 范围：引擎仅提供构造逆操作与回放序列的纯函数能力。日志增删管理、游标移动属于 Pinia store
* 历史状态公式：
  $$
  G_k = \text{replayToStep}(G_0, \text{entries}[0..k-1], k)
  $$

#### 核心概念

* `OperationLog = { entries: OperationLogEntry[], cursor: number }`
  * `cursor = k` — 当前状态为 $G_k$（已应用 entries[0..k-1]）
* 回溯（Undo）：对 entries[cursor-1] 执行逆操作序列 → cursor--
* 重做（Redo）：对 entries[cursor] 执行正向操作 → cursor++
* 截断：cursor 不为末尾时执行新操作，旧分支通过 reflog 保留引用
* 分支恢复：通过 reflog 找回被截断的操作分支

#### 回顾历史 $\neq$ 回溯

* 回顾历史：`replayToStep(G₀, entries, m)` — 临时 fork，不修改 GraphData，不动 cursor
...
| 服务提供接口（spi/persistence） | ✅ 接口定义 | 实现由消费者注入 |
