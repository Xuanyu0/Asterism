...
- **数据流**：层与值**交替**，行间以 `↓` 连接；`↓` 独占一行，上是输入、下是输出、右是该步的转换动作
  - 层上方的 `↓` 是它的入口，下方的是它的出口
  - 多条 `↓` 按列排布：**一列 = 一条管道**，同行并列 = 并行管道；读序行自上而下、行内自左而右
  - **不合并**：即便两条边传递同样的数据，也各画各的（确切范围待定，见细则 §7.1）
- **`§<层>: <实体节点>`** 引用已定义的层节点：同一层再次出现（反馈 / 逆层序）时重锚，不再重复 `≝` / `=` / 注解；也作为**列级来源锚点**写在某列 `↓` 正上方，声明该列的来源不是紧邻其上的层；未标锚点的列沿用上方层（`<层>` 须与图内层名逐字一致；`<实体节点>` 可省略；同一层的多个实体节点用 ` + ` 连接）
- 行末 `←` 表注解
