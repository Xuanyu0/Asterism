...
- 规则 1（目录项块）：块内条目名与续行 payload 对齐到统一列 P；
  P = 12（缩进）+ measure("├── ") + max(measure(条目名)) + 2。
...
def is_entry_line(line: str) -> bool:
    """目录项条目行：缩进 12 且以盒线 ├── / └── 开头。"""
    return indent_width(line) == ITEM_INDENT and line[ITEM_INDENT:].startswith(
        ("├──", "└──")
    )


def is_cont_line(line: str) -> bool:
    """目录项续行：缩进 ≥ 12 的非空白行，且不是条目行。

    架构语法允许省略引导用的 │（纯对齐续行），故不能以首字符 │ 作判据；
    缩进 ≥ 12 的非空行在此层级下非条目即续行。
    """
    return (
        indent_width(line) >= ITEM_INDENT
        and line.strip() != ""
        and not is_entry_line(line)
    )
...
def build_entry_line(box: str, name: str, payload: str, target_col: int) -> str:
...
def build_cont_line(marker: str, payload: str, target_col: int) -> str:
...
def plan_rule1_block(
    ...
    """为一个目录项块生成期望行与不合规明细。"""
    entry_idx = [k for k in range(start, end) if is_entry_line(lines[k])]
    if not entry_idx:
        return
    ...
    for k in range(start, end):
        ...
        if k in metas:
            ...
            rebuilt = build_entry_line(box, name, payload, target_col)
            ...
            rebuilt = build_cont_line(marker, payload, target_col)
...
def analyze(lines: list[str], offset: int) -> tuple[dict[int, str], list[Finding], int]:
    ...
    i = 0
    while i < len(lines):
        if is_entry_line(lines[i]):
            j = i
            while j < len(lines) and (is_entry_line(lines[j]) or is_cont_line(lines[j])):
                j += 1
            block_count += 1
            plan_rule1_block(lines, i, j, offset, expected, findings)
            i = j
        else:
            i += 1
