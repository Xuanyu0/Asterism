...
describe('internalize', () => {
    it('混合输入（知识节点迁移 + 引用节点自动删除）', () => {
        ...
        expect(result.operations.parent.some(op => op.type === 'delete_node')).toBe(true)
    ...
    it('常识层位置散布（scatterInCircle 递增）', () => {
        const graph = assembleGraph({ id: 'test-int-sct' as GraphId, nodes: [
            ...
            createNode({ id: 'k3' as NodeId, graphId: 'test-int-sct' as GraphId }),
        ], edges: [] })
        const common = createCommonLayerGraph()
        const result = internalize({
        ...
        expect(result.issues.filter(i => i.severity === 'error')).toHaveLength(0)
        expect(result.operations.commonLayer.filter(op => op.type === 'add_node')).toHaveLength(3)
        // position 各不相同（不碰撞）
