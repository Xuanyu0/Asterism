...
import type { EdgeId, GraphData, GraphId, GraphRegistry, NodeId } from '../../../src/types/graph_data'
...
function asInGraph(batch: OperationBatch): Extract<OperationBatch, { kind: 'inGraph' }> {
...
describe('deleteAbstractNode', () => {
    test('三层嵌套 + 沟通节点 + 原子混层：批序与职责归属', () => {
        ...
        expect(clearG2.operations).toEqual([{ type: 'delete_node', nodeId: 'C' }])
        ...
        const deleteIds = clearG1.operations.map((op) => (op as { type: 'delete_node'; nodeId: NodeId }).nodeId)
        ...
        expect(clearG0.operations).toEqual([{ type: 'delete_node', nodeId: 'A' }])
    })
    ...
    test('单层抽象：G1 清空 + G1 注销 + G0 清空', () => {
        ...
        const g1 = assembleGraph({
            ...
            parentGraphId: G0,
            ownerNodeId: 'A' as NodeId,
            nodes: [createNode({ id: 'D' as NodeId, graphId: G1 }), createNode({ id: 'E' as NodeId, graphId: G1 })],
            edges: [],
        })
    ...
    test('原子节点传入 → DELETE_ABSTRACT_TARGET_NOT_ABSTRACT', () => {
        const g0 = assembleGraph({
            id: G0,
            nodes: [createNode({ id: 'A' as NodeId, graphId: G0 }), createNode({ id: 'F' as NodeId, graphId: G0 })],
            edges: [],
        })
        ...
        expect(result.issues[0]?.code).toBe('DELETE_ABSTRACT_TARGET_NOT_ABSTRACT')
    })
