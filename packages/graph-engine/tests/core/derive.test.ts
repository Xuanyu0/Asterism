...
function makeKnowledgeNode(childGraphId?: GraphId, id: NodeId = 'n', graphId: GraphId = 'g'): KnowledgeNodeData {
    return {
        id,
        graphId,
        role: 'knowledge',
...
function makeReferenceNode(sourceGraphId: GraphId, sourceNodeId: NodeId): ReferenceNodeData {
    return {
        id: 'r',
        graphId: 'g',
        role: 'reference',
...
function makeLookup(graphs: GraphData[]): (graphId: GraphId) => GraphData | undefined {
...
describe('deriveNodeForm', () => {
    ...
    test('有 childGraphId → abstract', () => {
        expect(deriveNodeForm(makeKnowledgeNode('sub-a' as GraphId))).toBe('abstract')
    })
})
...
describe('deriveAbstractionLevel', () => {
    ...
    test('抽象节点空子图 → 1', () => {
        ...
        expect(deriveAbstractionLevel(lookup, makeKnowledgeNode('sub-a' as GraphId))).toBe(1)
    })

    test('深层链多级嵌套 → 递增', () => {
        // sub-c（含原子节点）⊂ sub-b ⊂ sub-a：入口 → sub-a → sub-b → sub-c → 原子
        const graphC = makeGraph('sub-c' as GraphId, [makeKnowledgeNode()])
        const graphB = makeGraph('sub-b' as GraphId, [makeKnowledgeNode('sub-c' as GraphId)])
        const graphA = makeGraph('sub-a' as GraphId, [makeKnowledgeNode('sub-b' as GraphId)])
        const lookup = makeLookup([graphC, graphB, graphA])

        expect(deriveAbstractionLevel(lookup, makeKnowledgeNode('sub-a' as GraphId))).toBe(3)
    })

    test('子图不可达（lookupGraph 返回 undefined）→ 防御 1', () => {
        const lookup = makeLookup([])
        expect(deriveAbstractionLevel(lookup, makeKnowledgeNode('missing' as GraphId))).toBe(1)
    })

    test('环检测：childGraphId 链成环时抛错（数据损坏）', () => {
        // sub-a 内节点指向 sub-b；sub-b 内节点指向 sub-a
        const graphA = makeGraph('sub-a' as GraphId, [makeKnowledgeNode('sub-b' as GraphId)])
        const graphB = makeGraph('sub-b' as GraphId, [makeKnowledgeNode('sub-a' as GraphId)])
        const lookup = makeLookup([graphA, graphB])

        expect(() => deriveAbstractionLevel(lookup, makeKnowledgeNode('sub-a' as GraphId))).toThrow(/成环/)
    })
    ...
    test('DAG 共享子图：同一子图被多节点引用不误判环', () => {
        ...
        const graphA = makeGraph('sub-a' as GraphId, [makeKnowledgeNode('sub-shared' as GraphId)])
        const graphB = makeGraph('sub-b' as GraphId, [makeKnowledgeNode('sub-shared' as GraphId)])
        const lookup = makeLookup([shared, graphA, graphB])

        // 入口 → sub-a → sub-shared → 原子 = 2
        expect(deriveAbstractionLevel(lookup, makeKnowledgeNode('sub-a' as GraphId))).toBe(2)
    })

    test('引用节点解引用源节点推导，结果与源节点一致', () => {
        const child = makeGraph('sub-src' as GraphId, [])
        const sourceNode = makeKnowledgeNode('sub-src' as GraphId, 'src-node' as NodeId, 'src-graph' as GraphId)
        const sourceGraph = makeGraph('src-graph' as GraphId, [sourceNode])
        const referenceNode = makeReferenceNode('src-graph' as GraphId, 'src-node' as NodeId)
        const lookup = makeLookup([child, sourceGraph])

        expect(deriveAbstractionLevel(lookup, referenceNode)).toBe(deriveAbstractionLevel(lookup, sourceNode))
    })
    ...
    test('源节点不可达 → 0', () => {
        ...
        expect(
            deriveAbstractionLevel(emptyLookup, makeReferenceNode('missing-graph' as GraphId, 'src-node' as NodeId)),
        ).toBe(0)
        ...
        expect(
            deriveAbstractionLevel(lookup, makeReferenceNode('src-graph' as GraphId, 'missing-node' as NodeId)),
        ).toBe(0)
    })
    ...
    test('递归内部引用节点（沟通节点）不参与层级，不抛环错误', () => {
        ...
        const communicationNode = makeReferenceNode('parent' as GraphId, 'node-a' as NodeId)
        const childGraph = makeGraph('child' as GraphId, [communicationNode])
        const nodeA = makeKnowledgeNode('child' as GraphId, 'node-a' as NodeId, 'parent' as GraphId)
