 ...
 * 单一维护，引擎批级契约校验与前端提交前的批次拆分均直接消费本模块两个守卫。
 */

import type { AtomicGraphOperation, AtomicOperation, AtomicOperationInGraph } from '../../types/atomic_operations'
 ...
 *
 * @param op - 跨图内 / 图级联合的操作
 * @returns true 表示 op 为图内操作（非 add_graph / delete_graph / update_graph）。
 */
export function isInGraphType(op: AtomicOperation): op is AtomicOperationInGraph {
